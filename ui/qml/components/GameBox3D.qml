import QtQuick
import QtQuick.Layouts
import QtQuick.Controls
import QtQuick.Effects

Item {
    id: root3d
    property string source: ""
    property color accentColor: "#4da6ff"
    property real tiltX: 0
    property real tiltY: 0
    property real pathRotation: 0 
    property bool showShadow: true
    property bool showGlow: false
    
    onShowGlowChanged: {
        if (!showGlow) {
            tiltX = 0
            tiltY = 0
        }
    }
    
    // Suavizado de la inclinación
    Behavior on tiltX { NumberAnimation { duration: 300 } }
    Behavior on tiltY { NumberAnimation { duration: 300 } }

    implicitWidth: 380
    implicitHeight: 540

    // 1. ÁREA DE INTERACCIÓN (HABILITADA PARA TODAS LAS TARJETAS)
    MouseArea {
        id: sensorArea
        anchors.fill: parent
        hoverEnabled: true
        z: 1000 
        enabled: root3d.showGlow
        
        onPositionChanged: (mouse) => {
            root3d.tiltX = (mouse.x / width) * 2 - 1
            root3d.tiltY = (mouse.y / height) * 2 - 1
        }
        onExited: {
            root3d.tiltX = 0
            root3d.tiltY = 0
        }
    }

    // 2. CONTENEDOR VISUAL
    Item {
        id: visualContent
        anchors.fill: parent
        
        // EFECTO 3D POTENCIADO: Inclinación por Mouse + Inclinación por Trayectoria
        property real combinedAngle: (-root3d.tiltX * 40) + root3d.pathRotation

        transform: [
            Rotation {
                origin.x: visualContent.width / 2
                origin.y: visualContent.height / 2
                axis { x: 1; y: 0; z: 0 }
                angle: root3d.tiltY * 20
            },
            Rotation {
                origin.x: visualContent.width / 2
                origin.y: visualContent.height / 2
                axis { x: 0; y: 1; z: 0 }
                angle: visualContent.combinedAngle
            }
        ]

        // SOMBRA
        Rectangle {
            anchors.horizontalCenter: parent.horizontalCenter
            anchors.bottom: parent.bottom
            anchors.bottomMargin: -45
            width: parent.width * 0.9
            height: 35
            radius: 100
            color: "black"
            opacity: root3d.showShadow ? 0.7 : 0
            layer.enabled: true
            layer.effect: MultiEffect { blurEnabled: true; blur: 1.0 }
            x: -root3d.tiltX * 25
            scale: 1.0 - (Math.abs(root3d.tiltX) * 0.2)
        }

        // BACKGLOW
        Rectangle {
            anchors.fill: parent
            anchors.margins: -10
            radius: 26
            color: "transparent"
            border.color: accentColor
            border.width: 10
            opacity: root3d.showGlow ? 0.45 : (sensorArea.containsMouse ? 0.25 : 0)
            layer.enabled: true
            layer.effect: MultiEffect { blurEnabled: true; blur: 0.8 }
            SequentialAnimation on opacity {
                running: root3d.showGlow
                loops: Animation.Infinite
                NumberAnimation { from: 0.25; to: 0.5; duration: 2500; easing.type: Easing.InOutSine }
                NumberAnimation { from: 0.5; to: 0.25; duration: 2500; easing.type: Easing.InOutSine }
            }
        }

        // CAJA
        Item {
            anchors.fill: parent
            
            // LOMO
            Rectangle {
                id: spine
                width: 35
                height: parent.height
                anchors.right: mainBox.left
                anchors.rightMargin: -1
                color: "#181a24"
                visible: visualContent.combinedAngle > 0.5
                gradient: Gradient {
                    GradientStop { position: 0.0; color: "#2d3142" }
                    GradientStop { position: 1.0; color: "#0a1018" }
                }
                transform: Rotation {
                    origin.x: 35
                    origin.y: parent.height / 2
                    axis { x: 0; y: 1; z: 0 }
                    angle: -90
                }
            }

            // MARCO PRINCIPAL
            Rectangle {
                id: mainBox
                anchors.fill: parent
                radius: 18
                color: "#0c0e16"
                border.color: "#33ffffff"
                border.width: 2.5
                clip: true

                Image {
                    anchors.fill: parent
                    anchors.margins: 12
                    source: root3d.source
                    fillMode: Image.PreserveAspectCrop
                    asynchronous: true
                    
                    Rectangle {
                        anchors.fill: parent
                        opacity: 0.5 * Math.abs(root3d.tiltX)
                        gradient: Gradient {
                            orientation: Gradient.Horizontal
                            GradientStop { position: 0.0; color: "transparent" }
                            GradientStop { position: 0.5; color: "#55ffffff" }
                            GradientStop { position: 1.0; color: "transparent" }
                        }
                        x: root3d.tiltX * 220
                    }
                }
                
                Rectangle {
                    anchors.fill: parent
                    radius: 18
                    opacity: 0.15
                    gradient: Gradient {
                        GradientStop { position: 0.0; color: "#ffffff" }
                        GradientStop { position: 0.2; color: "transparent" }
                        GradientStop { position: 1.0; color: "#000000" }
                    }
                }
            }
        }
    }
}
