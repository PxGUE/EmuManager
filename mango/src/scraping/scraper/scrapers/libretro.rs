use crate::scraping::scraper::scrapers::{ScrapedMetadata, ScrapeQuery, MetadataSource, download_image, clean_filename_for_search};
use reqwest::Client;
use std::path::{Path};
use std::fs;
use pyo3::prelude::*;
...
pub struct LibretroSource;

#[async_trait]
impl MetadataSource for LibretroSource {
    fn name(&self) -> &'static str { "Libretro" }

    async fn scrape(&self, query: &ScrapeQuery) -> Option<ScrapedMetadata> {
        Python::with_gil(|py| {
            crate::scraping::batch_scraper::log_to_python(py, "debug", &format!(
                "[LIBRETRO] Iniciando búsqueda para: '{}' (Plataforma: '{}')", 
                query.filename, query.platform
            ));
        });

        let repo_name = match query.platform.to_lowercase().as_str() {
            ...
            _ => {
                Python::with_gil(|py| {
                    crate::scraping::batch_scraper::log_to_python(py, "warning", &format!(
                        "[LIBRETRO] Plataforma no soportada en Libretro: '{}'", query.platform
                    ));
                });
                return None;
            }
        ...
        if let Some(match_result) = fuzzy::find_best_match(&clean_local, index, 0.85) {
            ...
            if let Ok(path) = download_image(&client, &target_url, &query.media_dir, "covers/2d", &file_stem).await {
