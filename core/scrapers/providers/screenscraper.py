...
class ScreenScraperScraper(BaseScraper):
    ...
    async def fetch(self, session: aiohttp.ClientSession, query: str, **kwargs) -> Optional[Dict[str, Any]]:
        # ðŸ›¡ï¸ GUARD: No intentamos nada si faltan credenciales crÃ­ticas o son los placeholders por defecto
        if not self.user or not self.password or self.devid in (None, "", "EMU_MANAGER_DEV"):
            # Omitimos log para no saturar si es lo esperado (sin configuraciÃ³n)
            return None
        ...
        try:
            ...
            async with session.get(self.API_BASE, params=params, timeout=12) as resp:
                if resp.status == 404:
                    # ðŸ”„ FALLBACK SILENCIOSO: Si falla por archivo, reintentamos por nombre de juego
                    params["romtype"] = "nom"
                    params["romnom"] = query 
                    async with session.get(self.API_BASE, params=params, timeout=12) as retry_resp:
            ...
            print(f"[SCREEN_SCRAPER] ExcepciÃ³n en fetch: {e}")
            pass
        return None
